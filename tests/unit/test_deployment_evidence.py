...
def test_paper_floor_can_pass_without_significant_edge_claim() -> None:
    ...
    report = evaluate_evidence(metrics)
    assert report.status is EvidenceStatus.PAPER_GATES_PASSED
    failed_significant = {gate.name for gate in report.failed_gates if gate.tier == "significant"}
    assert "significant_profit_factor" in failed_significant
    assert "pnl_without_top10" in failed_significant
    assert "ranking_spearman_pnl" in failed_significant
...
def test_missing_metric_fails_closed() -> None:
    ...
    assert report.status is EvidenceStatus.INSUFFICIENT_EVIDENCE
    gate = next(g for g in report.gates if g.name == "15m_brier")
    assert gate.passed is False


def test_profit_concentration_blocks_significant_claim_even_when_aggregate_is_good() -> None:
    report = evaluate_evidence(replace(_passing_metrics(), pnl_without_top10=-1.0))
    assert report.status is EvidenceStatus.PAPER_GATES_PASSED
    gate = next(g for g in report.gates if g.name == "pnl_without_top10")
    assert gate.passed is False
    assert gate.tier == "significant"
...
def test_inverted_ranking_blocks_significant_claim() -> None:
    report = evaluate_evidence(
        replace(
            ...
            ranking_spearman_win=-0.20,
        )
    )
    assert report.status is EvidenceStatus.PAPER_GATES_PASSED
    failed = {gate.name for gate in report.failed_gates}
    assert "ranking_spearman_pnl" in failed
    assert "ranking_spearman_win" in failed
